...
class PreviewCallback(PythonJavaClass):
    ...
    *returns*
    ...
    def __init__(self ,kivy_image_callback, recycle=True):
        super().__init__()
    ...
    def onPreviewFrame(self, data, camera):
        ...
        try:
            ...
            for obj in decoded_objects:
                ...
                content.append(obj.data.decode('utf-8'))
                toast(obj.data.decode('utf-8'))

        except Exception as e:
            print("Error decoding QR code:", e)
        
        # Recycle the previous frames
        if self.recycle:
            # recycle previous scanned frames/images
            [os.remove(image) for image in self.recycle_items if os.path.exists(image)]
            self.recycle_items.clear()
        # add the current image to the recycle items
        self.recycle_items.append(pic)
        # change kivy image
        self.kivy_image_callback(pic, content)
...
class AddPreviewRunnable(PythonJavaClass):
    ...
    def __init__(self, activity ,kivy_image_callback, recycle):
        super().__init__()
    ...
    def run(self):
        ...
        # add the callback
        self.preview_callback = PreviewCallback(self.kivy_image_callback , self.recycle)
...
class LegacyScanner:
    ...
    def __init__(self,image_callback , recycle_frames=True):
        """ 
        callback when a pic is taken 
        ``returned to image_callback``
        - ``captured frame path``
        - ``decoded content``

        ``recycle_frames: bool``
        If True, previous captured frames will be ``recycled (deleted)`` after processing.
        """
        self.image_callback = image_callback
        self.recycle_frames = recycle_frames
    ...
    def start_preview(self):
        ...
        if self.preview_runnable is None and check_permission(Permission.CAMERA):
            ...
            self.preview_runnable = AddPreviewRunnable(activity, self.image_callback , self.recycle_frames)
            activity.runOnUiThread(self.preview_runnable)
            return True
        
        elif self.preview_runnable and check_permission(Permission.CAMERA):
            print("Starting camera preview...")
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
            activity = PythonActivity.mActivity
            self.preview_runnable = AddPreviewRunnable(activity, self.image_callback , self.recycle_frames)
    ...
    def scan(self):
        """
        Scans the camera preview for QR codes or barcodes for once.
        """
        try:
            if self.preview_runnable: setattr(self.preview_runnable.preview_callback, "scan", True)
